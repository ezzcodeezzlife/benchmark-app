...
app.get('/:id', function(req, res, next) {
	...
	res.render('singlebench',  {id: req.params.id  } );
});

app.get('/profile/:id', async function(req, res, next) {
  var all = await dbBench.find({ 'nickname': req.params.id })
  console.log(all);
  
  res.render('profile', { tabledata: all, nickname: req.params.id, datajson: JSON.stringify(all) } );
});

app.get('/singleinfo/:id', async function(req, res, next) {
  let endpoint = "https://overpass.openstreetmap.fr/api/interpreter?data=[out:json]; (node(" + req.params.id + ");); (._;>;); out;"
  axios.get(endpoint)
  .then(response => {
